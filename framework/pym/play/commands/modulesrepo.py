...
def build(app, args, env):
    ...
    for (dirpath, dirnames, filenames) in os.walk(app.path):        
        if dirpath.find(os.sep + '.') > -1 or dirpath.find('/tmp/') > -1 or dirpath.find('/test-result/') > -1 or dirpath.find('/logs/') > -1 or dirpath.find('/eclipse/') > -1 or dirpath.endswith('/test-result') or dirpath.endswith('/logs') or dirpath.endswith('/eclipse') or dirpath.endswith('/nbproject') or dirpath.find('node_modules') > -1 or dirpath.find('data') > -1 :
            continue
