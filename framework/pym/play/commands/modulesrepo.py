...
def build(app, args, env):
    ...
    for (dirpath, dirnames, filenames) in os.walk(app.path):        
        if dirpath.find(os.sep + '.') > -1 or dirpath.find('/tmp/') > -1 or dirpath.find('/test-result/') > -1 or dirpath.find('/logs/') > -1 or dirpath.find('/eclipse/') > -1 or dirpath.startswith('/test-result') or dirpath.startswith('/logs') or dirpath.startswith('/eclipse') or dirpath.startswith('/nbproject') or dirpath.startswith('/node_modules') or dirpath.startswith('/data') :
            continue
